...
class CrossAttnUpBlock3D(nn.Module):
    def __init__(
        ...
        self.attentions_cp = copy.deepcopy(self.attentions)
        
        self.reset_stt = False
    ...
    def forward(
        ...
        if not self.reset_stt:
            self.attentions_cp = copy.deepcopy(self.attentions)
            # print(self.attentions_cp)
            # ipdb.set_trace(
            for name, params in self.attentions_cp.named_parameters():
                # print(name, params.requires_grad)
                # if name not in miss:
                if 'to_q.weight' in name:
        ...
        # TODO(Patrick, William) - attention mask is not used
        for resnet, temp_conv, attn, temp_attn, attn_cp in zip(
            self.resnets, self.temp_convs, self.attentions, self.temp_attentions, self.attentions_cp
        ):
            ...
            # I2V Adapter
            hidden_states_cp = attn_cp(
                hidden_states,
                encoder_hidden_states=encoder_hidden_states,
                cross_attention_kwargs=cross_attention_kwargs,
                return_dict=False,
            )[0]          
            hidden_states = hidden_states + hidden_states_cp
