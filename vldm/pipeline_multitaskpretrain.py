...
import PIL
import torch
from transformers import CLIPImageProcessor, CLIPTextModel, CLIPTokenizer, CLIPVisionModelWithProjection, AutoTokenizer, T5EncoderModel

from diffusers.image_processor import PipelineImageInput, VaeImageProcessor
from diffusers.models import AutoencoderKL
# from vldm.unet_i2vgen_xl_interpolation import I2VGenXLUNet
...
def tensor2vid(video: torch.Tensor, processor: "VaeImageProcessor", output_type: str = "np"):
    ...
    for batch_idx in range(batch_size):
        ...
        # print(batch_vid.shape) torch.Size([4, 3, 256, 256])
        # print(processor.do_denormalize)
        # exit()
...
@dataclass
class DFGNPipelineOutput(BaseOutput):
    r"""
...
def load_multiple_from_pretrained(paths: Dict[str, str]) -> Dict[str, object]:
    loaded_models = {}
    
    # Load models
    loaded_models['unet'] = DFGN.from_pretrained(paths['unet'], subfolder="unet")
    loaded_models['vae'] = AutoencoderKL.from_pretrained(paths['vae'], subfolder="vae")
    loaded_models['T5_tokenizer'] = AutoTokenizer.from_pretrained(paths['T5_tokenizer'])
    loaded_models['T5_text_encoder'] = T5EncoderModel.from_pretrained(paths['T5_text_encoder'])
    loaded_models['scheduler'] = DDIMScheduler.from_pretrained(paths['scheduler'], subfolder="scheduler")

    with open(paths['clip_text_encoder'], 'rb') as opened_file:
        checkpoint = torch.load(opened_file, map_location="cpu")
    cn_clip_model = clip.utils.create_model("ViT-H-14@RoBERTa-wwm-ext-large-chinese", checkpoint)
    cn_clip_model.float()
    loaded_models['clip_text_encoder'] = cn_clip_model
    
    return loaded_models

class DFGNPipeline(DiffusionPipeline):
    r"""
    ...
    def __init__(
        ...
        self.task_random_chosen = TaskRandomChosen()

    @classmethod
    def from_pretrained(cls, paths: Dict[str, str]) -> 'DFGNPipeline':
        loaded_models = load_multiple_from_pretrained(paths)

        return cls(
            unet=loaded_models['unet'],
            vae=loaded_models['vae'],
            scheduler=loaded_models['scheduler'],
            T5_tokenizer=loaded_models['T5_tokenizer'],
            T5_text_encoder=loaded_models['T5_text_encoder'],
            clip_text_encoder=loaded_models['clip_text_encoder'],
        )
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            text_input_ids = clip.tokenize(prompt, context_length = 42).to(device)
            PAD_INDEX = self.clip_text_encoder.tokenizer.vocab['[PAD]']
            ttn_mask = text_input_ids.ne(PAD_INDEX).to(device)
            ...
            prompt_embeds = self.clip_text_encoder.bert(text_input_ids, attention_mask=ttn_mask)[0]   
            T5_prompt_embeds = self.T5_text_encoder(T5_text_input_ids.to(device))[0]

        if self.clip_text_encoder is not None:
            prompt_embeds_dtype = self.clip_text_encoder.dtype
        ...
        T5_prompt_embeds = T5_prompt_embeds.to(dtype=prompt_embeds_dtype, device=device)
        ...
        # duplicate text embeddings for each generation per prompt, using mps friendly method
        T5_prompt_embeds = T5_prompt_embeds.repeat(1, num_videos_per_prompt, 1)
        T5_prompt_embeds = T5_prompt_embeds.view(bs_embed * num_videos_per_prompt, seq_len, -1)
        ...
        if self.do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            uncond_input = clip.tokenize(uncond_tokens, context_length = 42).to(device)
            uncond_ttn_mask = uncond_input.ne(PAD_INDEX).to(device)

            T5_uncond_input = self.T5_tokenizer(
                uncond_tokens,
                padding="max_length",
                max_length=42,
                truncation=True,
            ...
            negative_prompt_embeds = self.clip_text_encoder.bert(uncond_input, attention_mask=uncond_ttn_mask)[0]
            T5_negative_prompt_embeds = self.T5_text_encoder(T5_uncond_input.input_ids.to(device))[0]
        ...
        if self.do_classifier_free_guidance:
            ...
            seq_len = T5_negative_prompt_embeds.shape[1]
            T5_negative_prompt_embeds = T5_negative_prompt_embeds.to(dtype=prompt_embeds_dtype, device=device)
            T5_negative_prompt_embeds = T5_negative_prompt_embeds.repeat(1, num_videos_per_prompt, 1)
            T5_negative_prompt_embeds = T5_negative_prompt_embeds.view(batch_size * num_videos_per_prompt, seq_len, -1)

        return prompt_embeds, T5_prompt_embeds, negative_prompt_embeds, T5_negative_prompt_embeds
    ...
    def __call__(
        ...
        info_mask: List[int] = [1,0,0,0,0,0,0,0],
        frame_mask: List[int] = [1,1,1,1,0,0,0,0],
        ...
        prompt_embeds, T5_prompt_embeds, negative_prompt_embeds, T5_negative_prompt_embeds= self.encode_prompt(
            prompt,
            device,
            num_videos_per_prompt,
            negative_prompt,
        ...
        if self.do_classifier_free_guidance:
            ...
            T5_prompt_embeds = torch.cat([T5_negative_prompt_embeds, T5_prompt_embeds])
        ...
        # cropped_image = _center_crop_wide(pil_images, (width, width))
        ...
        # image_embeddings = image_embeddings.squeeze(dim=1).mean(dim=0).unsqueeze(dim=0)
        ...
        # image = self.image_processor.preprocess(resized_image).to(device=device)
        ...
        # 6.5
        # process image_latents
        image_latents = image_latents.unsqueeze(dim=0)
        b, c, f_src, h, w = image_latents.shape

        # 128, 3, 256, 256
        image_latents = image_latents.permute(0, 2, 1, 3, 4).reshape(b*f_src, c, h, w)

        # 128, 4, 32, 32
        image_latents = self.vae.encode(image_latents.to(torch.float32)).latent_dist.sample()
        image_latents = image_latents * self.vae.config.scaling_factor

        # 16, 4, 5, 32, 32
        image_latents = image_latents[None, :].reshape((b, f_src, -1) + image_latents.shape[2:]).permute(0, 2, 1, 3, 4)
        image_latents = torch.cat([image_latents] * 2) if self.do_classifier_free_guidance else image_latents

        # print('image_latents.shape', image_latents.shape)
        # print('info_mask', info_mask)
        frame_mask = [frame_mask]
        image_latents = self.task_random_chosen.get_masked_info(image_latents, info_mask, frame_mask)

        # image_embeddings = torch.cat([image_embeddings] * 2) if self.do_classifier_free_guidance else image_embeddings
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.unet(
                    ...
                    clip_hidden_states = prompt_embeds,
        ...
        if output_type == "latent":
            return DFGNPipelineOutput(frames=latents)
        ...
        return DFGNPipelineOutput(frames=video)
