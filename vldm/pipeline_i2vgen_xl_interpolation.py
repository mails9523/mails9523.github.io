...
class I2VGenXLPipeline(DiffusionPipeline):
    ...
    def _encode_image(self, image, device, num_videos_per_prompt=1):
    ...
    def __call__(
        ...
        # means = torch.tensor(mean_std[0], dtype=torch.float32)
        # stds = torch.tensor(mean_std[1], dtype=torch.float32)
        # unnormalize = T.Normalize((-means / stds).tolist(), (1.0 / stds).tolist())
        # b, c, f, h, w = latents.shape
        # after_unnormalize = []
        # for i in range(b):
        #     after_unnormalize_tmp = []
        #     for j in range(f):
        #         t = latents[i, :, j]
        #         after_unnormalize_tmp.append(unnormalize(t).unsqueeze(dim=1))
        #     after_unnormalize_tmp = torch.cat(after_unnormalize_tmp, dim=1)
        #     after_unnormalize.append(after_unnormalize_tmp.unsqueeze(dim=0))
        # latents = torch.cat(after_unnormalize, dim=0)
