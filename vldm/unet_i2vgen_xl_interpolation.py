...
class I2VGenXLUNet(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def forward(
        ...
        # print(image_latents_context_embs.shape)
        ...
        # image_latents = (
        #     image_latents[None, :]
        #     .reshape(batch_size, num_frames_c, channels, height, width)
        #     .permute(0, 3, 4, 1, 2)
        #     .reshape(batch_size * height * width, num_frames_c, channels)
        # )
        # image_latents = self.image_latents_temporal_encoder(image_latents)
        # image_latents = image_latents.reshape(batch_size, height, width, num_frames_c, channels).permute(0, 4, 3, 1, 2)
...
if __name__ == "__main__":
    ...
    noisy_latents = torch.rand((1, 4, 8, 32, 32)).cuda()

    timesteps = torch.tensor([8]).cuda()

    encoder_hidden_states = torch.rand((1, 77, 1024)).cuda()

    fps_tensor = torch.tensor([1]).cuda()

    image_latents = torch.rand((1, 4, 8, 32, 32)).cuda()

    image_embeddings = torch.rand((1, 1, 1024)).cuda()
