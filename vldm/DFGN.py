...
# )
try:
    from vldm.unet_3d_blocks import (
        CrossAttnDownBlock3D,
        CrossAttnUpBlock3D,
        DownBlock3D,
        UNetMidBlock3DCrossAttn,
...
class DFGN(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    r"""
    DFGN UNet. It is a conditional 3D UNet model that takes a noisy sample, conditional state, and a timestep
    ...
    def __init__(
        ...
        # T5 linear
        self.context_T5_clip_embedding = nn.Sequential(
            nn.Linear(t5_dim, 2 * cross_attention_dim),
            nn.SiLU(),
            nn.Linear(2 * cross_attention_dim, cross_attention_dim),
        )
        ...
        self.image_latents_temporal_encoder = TransformerTemporalEncoder(
            dim=in_channels,
        ...
        # other embeddings -- time, context, etc.
        ...
        for i, down_block_type in enumerate(down_block_types):
            ...
            down_block = get_down_block(
                ...
                # attn_num_head_channels=num_attention_heads[i],
        ...
        for i, up_block_type in enumerate(up_block_types):
            ...
            up_block = get_up_block(
                ...
                # attn_num_head_channels=reversed_num_attention_heads[i],
                dual_cross_attention=False,
                resolution_idx=i,
            )
    ...
    def forward(
        self,
        sample: torch.FloatTensor,  # [1, 4, 8, 32, 32]
        timestep: Union[torch.Tensor, float, int],
        image_latents: torch.Tensor, # [1, 4, 8, 32, 32]
        clip_hidden_states: Optional[torch.Tensor] = None, # [1, 77, 1024]
        t5_hidden_states: Optional[torch.Tensor] = None, # [1, 77, 768]
        timestep_cond: Optional[torch.Tensor] = None,
        cross_attention_kwargs: Optional[Dict[str, Any]] = None,
        return_dict: bool = True,
    ) -> Union[UNet3DConditionOutput, Tuple[torch.FloatTensor]]:
        r"""
        The [`DFGN`] forward method.
        ...
        # 4. text embeddings
        t5_hidden_states = self.context_T5_clip_embedding(t5_hidden_states)
        context_emb = torch.cat([t5_hidden_states, clip_hidden_states], dim=1)
        context_emb = context_emb.repeat_interleave(repeats=num_frames, dim=0)
        # print(context_emb.shape) # torch.Size([8, 141, 1024]
        ...
        # print(image_latents.shape) # torch.Size([1, 4, 8, 32, 32])
        # print(sample.shape) #torch.Size([1, 4, 8, 32, 32])
        # exit()
...
def cnt_net_params(net):
    ...
    print(f"Paramas: {round(net_params / (1000 * 1000 * 1000), 3)} B")
...
if __name__ == "__main__":
    ...
    noisy_latents = torch.rand((1, 4, 8, 32, 32))

    timesteps = torch.tensor([8])

    clip_hidden_states = torch.rand((1, 77, 1024))
    t5_hidden_states = torch.rand((1, 77, 768))

    image_latents = torch.rand((1, 4, 8, 32, 32))
    ...
    model_pred = unet(
        noisy_latents, 
        timesteps, 
        clip_hidden_states = clip_hidden_states,
        t5_hidden_states = t5_hidden_states,
        image_latents = image_latents,
        ).sample  # torch.Size([1, 4, 8, 32, 32])
